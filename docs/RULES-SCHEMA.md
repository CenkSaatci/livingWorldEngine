...
- **Preise:** `pricing: "local"` skaliert alle `money`-Beträge einer Aktion mit dem Ortswohlstand (dieselbe Formel wie der Händlermarkt), `priceModifier` (optional, > 0) multipliziert zusätzlich. Ohne `pricing` bleiben Beträge fix. Anzeige-Kosten und Wirkung nutzen denselben Faktor.
