...
> Stand Phase 33 + Audits. Backend-Tests: `mvn -B test` (432); Frontend: `npx vitest run` (185); E2E: `cd frontend && npm run test:e2e` (Playwright, 10 Szenarien + Login-Setup — inkl. Adventure-Inject — Login-Setup, Wizard-Pakete, Charakter-Wizard, Sheet-DSA, Kampf-Manöver, Kampf-Zustände/Schadensart, DM-Queue, System-Pin, Save-Gate, Dashboard-Smoke). Voraussetzung: pm2 `lwe-frontend`/`lwe-backend` laufen, `npx playwright install chromium` einmalig; Zugang via `E2E_EMAIL`/`E2E_PASSWORD` (Default devbe).
