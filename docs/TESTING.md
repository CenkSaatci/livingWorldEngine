...
> Stand P31 + Audits. Backend-Tests: `mvn -B test` (362); Frontend: `npx vitest run` (168); E2E: `cd frontend && npm run test:e2e` (Playwright, 7 Tests — Login-Setup, Wizard-Pakete, Charakter-Wizard, Sheet-DSA, Kampf-Manöver, Save-Gate, Dashboard-Smoke). Voraussetzung: pm2 `lwe-frontend`/`lwe-backend` laufen, `npx playwright install chromium` einmalig; Zugang via `E2E_EMAIL`/`E2E_PASSWORD` (Default devbe).
