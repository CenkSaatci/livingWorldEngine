...
describe('StatusBar', () => {
  ...
  it('shows "no session" by default', () => {
    useWorldStore.setState({ currentWorld: mockWorld });
    const { container } = renderStatusBar();
    expect(container.textContent).toContain('Keine Session');
  });
  ...
  it('shows active session when set', () => {
    ...
    const { container } = renderStatusBar();
    expect(container.textContent).toContain('Session aktiv');
  });
  ...
  it('shows DM controls for admin users', () => {
    ...
    expect(screen.getByLabelText('Nebel des Krieges umschalten')).toBeTruthy();
    expect(screen.getByLabelText('Zeit pausieren oder fortsetzen')).toBeTruthy();
    expect(screen.getByLabelText('Zeit vorspulen')).toBeTruthy();
  });
  ...
  it('hides DM controls for non-admin users', () => {
    ...
    renderStatusBar();
    expect(screen.queryByLabelText('Nebel des Krieges umschalten')).toBeNull();
    expect(screen.queryByLabelText('Zeit pausieren oder fortsetzen')).toBeNull();
    expect(screen.queryByLabelText('Zeit vorspulen')).toBeNull();
  });
  ...
  it('shows advance time options on click', async () => {
    ...
    useAuthStore.setState({ user: mockUser, isAuthenticated: true });
    renderStatusBar();
    const advanceBtn = screen.getByLabelText('Zeit vorspulen');
