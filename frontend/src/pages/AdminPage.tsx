...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get('/admin/worlds/stats').then((r) => setStats(r.data)).catch(() => {});
  }, []);

  if (user?.role !== 'ADMIN') {
    return <div className="p-6 text-text-secondary">Access denied</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      <header className="flex items-center gap-3 border-b border-bg-elevated bg-bg-surface px-6 py-3">
        ...
        <h1 className="text-lg font-heading text-text-primary">Admin</h1>
      </header>
      ...
      <main className="mx-auto max-w-4xl space-y-6 p-6">
        ...
        <section className="rounded-lg bg-bg-surface p-5">
          ...
          <div className="space-y-2">
            {users.map((u) => (
              <div key={u.id} className="flex items-center justify-between rounded bg-bg-primary/50 px-3 py-2">
                <div>
